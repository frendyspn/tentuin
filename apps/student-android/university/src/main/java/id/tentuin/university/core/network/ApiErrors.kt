...
fun friendlyAuthError(t: Throwable): String = when (t) {
    is HttpException -> {
        ...
        when {
            ...
            t.code() == 400 && desc.contains("email not confirmed") -> "Email belum diverifikasi."
            t.code() == 400 && desc.contains("password") -> "Password minimal 6 karakter."
            t.code() == 400 && desc.contains("email") -> "Format email tidak valid."
            t.code() == 401 -> "Akun tidak ditemukan atau sesi berakhir. Silakan login ulang."
            ...
            t.code() == 422 -> "Data tidak valid: ${parsed?.error_description ?: parsed?.msg ?: "periksa input"}"
            t.code() == 429 -> "Terlalu banyak percobaan. Coba lagi beberapa menit."
            t.code() in 500..599 -> "Server sedang bermasalah. Coba lagi beberapa saat."
            else -> parsed?.error_description ?: parsed?.msg ?: parsed?.message ?: "Gagal memproses (HTTP ${t.code()})."
        }
    }
...
fun friendlyApiError(t: Throwable, default: String = "Gagal memuat data."): String = when (t) {
    is HttpException -> {
        val parsed = parseSupabaseError(t)
        val pgCode = parsed?.code
        when {
            pgCode == "23505" -> "Data sudah ada (duplikat)."
            pgCode == "23503" -> "Data referensi tidak ditemukan: ${parsed.details ?: parsed.message ?: "cek data"}"
