...
import id.tentuin.agent.data.model.RefreshRequest
...
class AuthInterceptor @Inject constructor(
    ...
    private val apiProvider:      Provider<AgentApi>,
) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        // 401 with an active token → likely JWT expired. Try a single refresh + retry.
        response.close()
    ...
    private suspend fun tryRefresh(staleToken: String): Boolean = refreshMutex.withLock {
        // Another concurrent request may have refreshed already — re-read & short-circuit.
